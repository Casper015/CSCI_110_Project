...
void displayPayoutTable()
{
	...
	cout << setw(wLabel) << left << "1 bananna" << setw(wValue) << right << "1" << endl;
	cout << endl;
...
string getRandomSymbol()
{
	...
	int n = rand() % 120;
	if (n > 100) symbol = "bananna";
	else if (n > 90) symbol = "4";
...
int getPayoutRatio()
{
	...
	// Single cherry
		...
		payoutRatio = 2;
	// Single Bananna
	else if (isAny("bananna", Symbol1, Symbol2, Symbol3))
		payoutRatio = 1;
...
void showStatistics()
{
	...
	string s1, s2, s3;
	int triple4 = 0, tripleHat = 0, triple7 = 0, tripleBar = 0, tripleCherry = 0, doubleCherry = 0, singleCherry = 0, singleBananna = 0;
	for (int i = 0; i < iterations; i++)
	{
		...
		if (isThreeOfAKind("4", s1, s2, s3))
			...
			singleCherry++;
		else if (isAny("bananna", s1, s2, s3))
			singleBananna++;
	}
	...
	cout << "1 bananna: " << (100.0 * singleBananna / iterations) << "%" << endl;
	cout << endl;
	cout << "Press ENTER to continue...";
	string s;
	getline(cin, s);
